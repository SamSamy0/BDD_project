...
class EvalView(View):
    ...
    def initView(self):
        self.grid_columnconfigure(0, weight=1)  # Make the columns fill all the space
        self.grid_rowconfigure(1, weight=1)

        self.title_label = ctk.CTkLabel(
            self, text="Evaluations", font=ctk.CTkFont(size=20, weight="bold")
        )
        self.title_label.grid(row=0, column=0, padx=20, pady=20)

        # liste des résumés scrollable
        self.scroll_frame = ctk.CTkScrollableFrame(
            self, label_text="Liste des évaluations"
        )
    ...
    def displayEvaluations(self):
        ...
        for auteur, note, id_eval in self.evaluations:
            ...
            btn = ctk.CTkButton(
                ...
                text="voir",
                width=60,
                command=lambda i=id_eval: self.eval_action(i),
            )
    ...
    def displayEval(self, auteur, commentaire, note):
        ...
        # Title and Author
        ctk.CTkLabel(popup, text=f"par {auteur}", text_color="gray").pack(
            padx=20, pady=(0, 10)
        )
        # Separator
        ...
        # Average
        ctk.CTkLabel(popup, text=f"Note moyenne : ★ {note}/5", text_color="gray").pack(
            padx=20, pady=(0, 15)
        )
